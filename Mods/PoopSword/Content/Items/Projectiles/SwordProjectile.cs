...
namespace PoopSword.Content.Items.Projectiles
{
	public class SwordProjectile : ModProjectile
	{
		...
		public override void AI() {
    		...
    		int dustIndex = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.FartInAJar);
    
    		// Modify dust properties
    		Main.dust[dustIndex].velocity *= 0.5f; // Slows down the dust movement
    		Main.dust[dustIndex].scale = 1.2f; // Increases dust size
    		Main.dust[dustIndex].noGravity = true; // Makes dust float
