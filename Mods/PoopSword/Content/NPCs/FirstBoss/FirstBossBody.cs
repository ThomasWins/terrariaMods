using PoopSword.Content.BossBars;
using PoopSword.Common.Systems;
//using PoopSword.Content.Items.Armor.Vanity;
//using PoopSword.Content.Items.Consumables;
//using PoopSword.Content.Pets.MinionBossPet;
...
namespace PoopSword.Content.NPCs.FirstBoss
{
    [AutoloadBossHead]
    public class FirstBossBody : ModNPC{
        ...
        public ref float SecondStageTimer_SpawnPoop => ref NPC.localAI[3];

        public static int MinionType() {
			return ModContent.NPCType<FirstBossMinion>();
		}
		...
		public static int MinionCount() {
			...
			if (Main.expertMode) {
				count += 20; // Increase by 5 if expert or master mode
			}
        ...
        public override void SetStaticDefaults()
        {
			...
			NPCID.Sets.NPCBestiaryDrawModifiers drawModifiers = new NPCID.Sets.NPCBestiaryDrawModifiers() {
				CustomTexturePath = "PoopSword/Assets/Textures/Bestiary/FirstBoss_Preview",
        ...
        public override void SetDefaults()
        {
			...
			// Custom boss bar
			NPC.BossBar = ModContent.GetInstance<FirstBossBossBar>();

			// The following code assigns a music track to the boss in a simple way.
			if (!Main.dedServ) {
				Music = MusicLoader.GetMusicSlot(Mod, "Assets/Music/Spiff-Spacely");
			}
        }
        ...
        public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) {
			...
			bestiaryEntry.Info.AddRange(new List<IBestiaryInfoElement> {
				...
				new FlavorTextBestiaryInfoElement("Poop boss, there really isnt much else to it.")
			});
		}
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot) {
			...
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.Placeable.Furniture.FirstBossTrophy>(), 10));
			npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<Items.BossDrops.MoldablePoo>(), 2));
...
//			notExpertRule.OnSuccess(ItemDropRule.Common(ModContent.ItemType<FirstBossMask>(), 7));
...
//			npcLoot.Add(ItemDropRule.BossBag(ModContent.ItemType<FirstBossBag>()));

			// ItemDropRule.MasterModeCommonDrop for the relic
//			npcLoot.Add(ItemDropRule.MasterModeCommonDrop(ModContent.ItemType<Items.Placeable.Furniture.FirstBossRelic>()));

			// ItemDropRule.MasterModeDropOnAllPlayers for the pet
//			npcLoot.Add(ItemDropRule.MasterModeDropOnAllPlayers(ModContent.ItemType<FirstBossPetItem>(), 8));
		}

        public override void OnKill() {

			// This sets downedMinionBoss to true, and if it was false before, it initiates a lantern night
			NPC.SetEventFlagCleared(ref DownedBossSystem.downedFirstBoss, -1);
        ...
        public override void HitEffect(NPC.HitInfo hit) {
			...
			if (NPC.life <= 0) {
				...
				int backGoreType = Mod.Find<ModGore>("FirstBossBody_Back").Type;
				int frontGoreType = Mod.Find<ModGore>("FirstBossBody_Front").Type;
                ...
                // Boss death sound
				SoundEngine.PlaySound(new SoundStyle($"PoopSword/Assets/Sounds/NPCKilled/MyBossDeath"), NPC.Center);
        ...
        private void SpawnMinions() {
			...
			for (int i = 0; i < count; i++) {
				NPC minionNPC = NPC.NewNPCDirect(entitySource, (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<FirstBossMinion>(), NPC.whoAmI);
				...
				FirstBossMinion minion = (FirstBossMinion)minionNPC.ModNPC;
        ...
        private void CheckSecondStage() {
			...
			foreach (var otherNPC in Main.ActiveNPCs) {
				if (otherNPC.type == MinionType() && otherNPC.ModNPC is FirstBossMinion minion) {
        ...
        private void DoSecondStage(Player player) {
			...
			DoSecondStage_SpawnPoop(player);
        ...
        private void DoSecondStage_SpawnPoop(Player player) {
			...
			SecondStageTimer_SpawnPoop++;
			if (SecondStageTimer_SpawnPoop > timerMax) {
				SecondStageTimer_SpawnPoop = 0;
			}

			if (NPC.HasValidTarget && SecondStageTimer_SpawnPoop == 0 && Main.netMode != NetmodeID.MultiplayerClient) {
				...
				int type = ModContent.ProjectileType<FirstBossPoop>();
